export default class CodeEditor {
    ...
    create() {
        ...
        // Initial Code
        editor.innerText = `onUpdate: function(dt, objects, runtime) {
    // 1. Control specific object by it's Friendly Name (OviHub)
    const hero = runtime.findByName('Hero');
    if (hero) {
        hero.rotation += 2 * dt;
    }
    ...
    setValue(text) {
        if (this.editor) {
            ...
            const lineNumbers = this.editor.previousSibling;
            if (lineNumbers) {
                lineNumbers.innerHTML = Array(lines).fill(0).map((_, i) => i + 1).join('<br>');
            }

...
export default class VectorInspector {
    static render(engine, editorInstance, selectedElement) {
        ...
        setTimeout(() => {
            ...
            if (starPoints) starPoints.oninput = updateStar;
            if (starInner) starInner.oninput = updateStar;
            ...
            if (polyPoints) polyPoints.oninput = updatePoly;
            ...
            if (arrowShaft) arrowShaft.oninput = updateArrow;
            if (arrowHead) arrowHead.oninput = updateArrow;
            ...
            if (pieStart) pieStart.oninput = updatePie;
            if (pieEnd) pieEnd.oninput = updatePie;
            if (pieInner) pieInner.oninput = updatePie;
            ...
            if (spTurns) spTurns.oninput = updateSpiral;
            if (spInner) spInner.oninput = updateSpiral;
            ...
            if (wvFreq) wvFreq.oninput = updateWave;
            if (wvAmp) wvAmp.oninput = updateWave;
            ...
            if (grRows) grRows.oninput = updateGrid;
            if (grCols) grCols.oninput = updateGrid;
            ...
            if (crArms) crArms.oninput = updateCross;
            if (crThick) crThick.oninput = updateCross;
            ...
            if (cresThick) cresThick.oninput = updateCres;
            if (cresStyle) cresStyle.onchange = updateCres;
            ...
            if (blComplex) blComplex.oninput = () => updateBlob(false);
            if (blContrast) blContrast.oninput = () => updateBlob(false);
            if (blRand) blRand.onclick = () => updateBlob(true);
            ...
            if (shShoulder || shCrest || shCurve) {
                ...
                if (shShoulder) shShoulder.oninput = updateShield;
                if (shCrest) shCrest.oninput = updateShield;
                if (shCurve) shCurve.oninput = updateShield;
            }
            ...
            if (geTeeth) geTeeth.oninput = updateGear;
            if (geDepth) geDepth.oninput = updateGear;
            if (geHole) geHole.oninput = updateGear;
            ...
            if (flPetals) flPetals.oninput = updateFlower;
            if (flRound) flRound.oninput = updateFlower;
            ...
            if (clBumps) clBumps.oninput = updateCloud;
            if (clPuff) clPuff.oninput = updateCloud;
            if (clIrreg) clIrreg.oninput = updateCloud;
            if (clFlat) clFlat.oninput = updateCloud;
            ...
            const updateWarp = () => {
                const type = warpTypeSelect.value;
                const bend = parseInt(warpBend.value);
                const distH = parseInt(warpDistH.value);
                const distV = parseInt(warpDistV.value);

                document.getElementById('vp-bend-val').textContent = bend + '%';
                document.getElementById('vp-dist-h-val').textContent = distH + '%';
                document.getElementById('vp-dist-v-val').textContent = distV + '%';
            ...
            if (warpTypeSelect) warpTypeSelect.onchange = updateWarp;
            if (warpBend) warpBend.oninput = updateWarp;
            if (warpDistH) warpDistH.oninput = updateWarp;
            if (warpDistV) warpDistV.oninput = updateWarp;

...
export default class VectorEditor {
    ...
    saveState() {
        ...
        // Clone current SVG content
        const children = Array.from(this.svg.children).filter(el => {
            // Exclude temporary UI elements
            if (el === this.gizmoGroup || el === this.marqueeRect || el.classList.contains('shape-builder-overlay')) return false;
            return true;
        });

        // Create a temporary container to generate innerHTML without the UI tools
        const temp = document.createElementNS(this.svgNs, "svg");
        children.forEach(child => temp.appendChild(child.cloneNode(true)));
    ...
    loadState(snapshot) {
        // 1. Clear current elements but keep tools
        ...
        // We set innerHTML of a temp div to parse the SVG fragments
        const temp = document.createElement('div');
        temp.innerHTML = `<svg xmlns="${this.svgNs}">${snapshot}</svg>`;
        const newSvg = temp.firstChild;

        Array.from(newSvg.children).forEach(child => {
            this.svg.insertBefore(child, this.gizmoGroup);
        });
    ...
    bindEvents() {
        ...
        this.svg.addEventListener('mouseup', () => {
            ...
            if (this.isDragging || this.isResizing || this.isWarpDragging) {
                this.saveState(); // SAVE: Transform/Warp
            }
    ...
    booleanOperation(op) {
        ...
        this.select(newEl);
        this.saveState(); // SAVE: Boolean Result
    }
    ...
    finalizeShapeBuilder() {
        ...
        try {
            ...
            if (newElements.length > 0) {
                ...
                this.updateGizmo();
                this.saveState(); // SAVE: Shape Builder Result
            } else {
                this.deselect();
            }
    ...
    ungroupSelection() {
        ...
        this.deselect();
        this.saveState(); // SAVE: Ungrouping
    }
    ...
    bringToFront() {
        ...
        this.updateGizmo();
        this.saveState(); // SAVE: Order Change
    }
    ...
    importSVG(svgContent) {
        ...
        this.svg.innerHTML = '';
        this.saveState(); // SAVE: Clear for Import
        ...
        this.saveState(); // SAVE: Finished Import

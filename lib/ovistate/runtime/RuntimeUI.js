        ...
        if (control.type === 'button') {
            ...
            content = `
                <div style="background: rgba(30,30,30,0.8); padding: 5px; border-radius: 4px; color: white; font-family: sans-serif; font-size: 12px; display: flex; align-items: center; gap: 6px;">
                    <input type="color" value="${control.value || '#ff0000'}" style="width: 30px; height: 30px; padding: 0; border: none; cursor: pointer;">
                    ...
                    switch (control.binding.action) {
                        ...
                        case 'toggle_behavior':
                            if (control.binding.actionId) {
                                ...
                                if (target._behaviorState[bId] === undefined) target._behaviorState[bId] = false;

                                if (control.binding.action === 'start_behavior') target._behaviorState[bId] = true;
                                if (control.binding.action === 'stop_behavior') target._behaviorState[bId] = false;
                                if (control.binding.action === 'toggle_behavior') {
                                    target._behaviorState[bId] = !target._behaviorState[bId];
                                }

                                console.log(`⚡ Behavior ${bId} state set to ${target._behaviorState[bId]} on ${target.id}`);
                            }
                            break;
                    }
                }

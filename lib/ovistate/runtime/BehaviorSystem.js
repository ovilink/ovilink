...
import { registerBrimBehaviors } from '../behaviors/BrimBehaviors.js';
import { registerPhysicsBehaviors } from '../behaviors/PhysicsBehaviors.js';
import { registerLogicBehaviors } from '../behaviors/LogicBehaviors.js';
...
export default class BehaviorSystem {
    ...
    registerAllBehaviors() {
        ...
        registerBrimBehaviors(this.registry);
        registerPhysicsBehaviors(this.registry);
        registerLogicBehaviors(this.registry);
    ...
    execute(obj, dt) {
        ...
        obj.behaviors.forEach(behaviorId => {
            ...
            if (registryBehavior && registryBehavior.update) {
                ...
                // DEBUG: Trace activation logic
                if (Math.random() < 0.005) { // Log occasionally to avoid spam
                    console.log(`⚡ BehaviorSystem: ${obj.id} [${behaviorId}] Mode: ${actMode}`, {
                        isHovered: obj.isHovered,
                        justClicked: obj._justClicked,
                        state: obj._behaviorState?.[behaviorId]
                    });
                }
                ...
                if (actMode === 'on_enter') {
                    ...
                    // --- NEW: Allow behaviors to stay active for reset transitions ---
                    if (obj.isHovered) obj._behaviorState[behaviorId] = true;
                    ...
                    // Manual is controlled by global events (activeActions Set)
                    if (this.runtime.activeActions && this.runtime.activeActions.has(actId)) {
                        obj._behaviorState[behaviorId] = true;
                    }
                    shouldRun = obj._behaviorState[behaviorId];
                }

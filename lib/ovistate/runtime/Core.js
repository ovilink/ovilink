...
export default class OviStateRuntime {
    ...
    getWorldTransform(obj, visited = new Set()) {
        if (visited.has(obj.id)) {
            console.warn(`[Core] Circular parent detected for ${obj.id}`);
            return { x: obj.x || 0, y: obj.y || 0, rotation: obj.rotation || 0, scale: obj.scale !== undefined ? obj.scale : 1 };
        }
        visited.add(obj.id);
        ...
        if (obj.parent) {
            ...
            if (parent) {
                const parentTransform = this.getWorldTransform(parent, visited);
        ...
        // REMOVED: renderOffset block (Double Application Fix)
        // renderOffset should only be applied in RENDER context shift, not World Transform
    ...
    getObjectBounds(obj) {
        ...
        if (obj.type === 'group') {
            ...
            // Default bounds based on width/height
            // For vector_path, if points are missing, we rely on width/height
            let w = obj.width || 0;
            let h = obj.height || 0;

            // Robust bounds for path/vector_path if they have points
            if ((obj.type === 'path' || obj.type === 'vector_path') && obj.points && obj.points.length > 0) {
        ...
        // Include children (Hierarchical)
        const children = this.objects.filter(o => o.parent === obj.id);
        if (children.length > 0) {
            children.forEach(child => {
                const cb = this.getObjectBounds(child);
                if (cb.minX === Infinity) return;

                const s = child.scale !== undefined ? child.scale : 1;
                const rot = (child.rotation || 0) * Math.PI / 180;
                const cos = Math.cos(rot);
                const sin = Math.sin(rot);

                // Child's local corners
                const corners = [
                    { x: cb.minX * s, y: cb.minY * s },
                    { x: cb.maxX * s, y: cb.minY * s },
                    { x: cb.maxX * s, y: cb.maxY * s },
                    { x: cb.minX * s, y: cb.maxY * s }
                ];

                corners.forEach(p => {
                    // Rotate and Translate to parent space
                    const rx = (p.x * cos - p.y * sin) + (child.x || 0);
                    const ry = (p.x * sin + p.y * cos) + (child.y || 0);
        ...
        if (minX === Infinity) return { minX: -20, minY: -20, maxX: 20, maxY: 20 };
        ...
        this.objects.slice().sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0)).forEach(obj => {
            ...
            if (obj.type === 'vector_path') {
                ...
                if (obj._path2d) {
                    ...
                    let aly = ly;
                    /* 
                    // FIX: This seems to double-apply the offset since Hit Test lx is already in World Space 
                    // which includes renderOffset via getWorldTransform?
                    // If getWorldTransform applies offset, lx is offset.
                    // If we offset again, we break it.
                    if (obj.renderOffset) {
    ...
    update(dt) {
        ...
        */
        ...
        // 0. Procedural Updates (for objects like ripples)
        this.objects.forEach(obj => {
            if (obj.update && typeof obj.update === 'function') {
                try { obj.update(dt); } catch (e) { }
            }
        });
        ...
        if (this.timelineSystem) this.timelineSystem.update(dt);
        this.particleSystem.update(dt, this.objects);
        if (this.ui) this.ui.update(dt);

        // Clear actions at the end of frame so they are visible to all systems during the frame
    ...
    setBehaviorKeyState(target, behaviorKey, state) {
        ...
        // 2. Event ID (Activation ID) Match
        if (target._behaviorState) {
            for (const bId in target._behaviorState) {
                const actId = this.registry ? this.registry.getParameter(target, bId, 'activationId') : undefined;
                if (actId === behaviorKey) {
    ...
    getRecursiveOpacity(obj) {
        try {
            let opacity = obj.opacity !== undefined ? obj.opacity : 1;
            let p = obj.parent;
            let steps = 0;
            while (p && steps < 50) {
                // Safety check for getObject existence
                if (typeof this.getObject !== 'function') break;

                const parent = this.getObject(p);
                if (parent) {
                    const pOp = parent.opacity !== undefined ? parent.opacity : 1;
                    opacity *= pOp;
                    p = parent.parent;
                } else {
                    break;
    ...
    render() {
        ...
        this.objects.sort((a, b) => (a.zIndex || 0) - (b.zIndex || 0)).forEach(obj => {
            ...
            // Debug: Trace faulty objects
            if (isNaN(world.x) || isNaN(world.y)) {
                console.error(`[Core Msg] NaN World Transform for ${obj.id}`, { obj, world });
                return;
            }
            ...
            // Apply opacity
            // Apply opacity (Recursive)
            try {
                const finalOpacity = this.getRecursiveOpacity(obj);
                this.ctx.globalAlpha = finalOpacity;
            } catch (e) {
                this.ctx.globalAlpha = 1;
            }

            // Draw selection handles BEFORE renderOffset if selected
            ...
            if (obj.type === 'sprite') {
                ...
                if (!this.isEditor && obj.showInExport === false) { this.ctx.restore(); return; }
                if (obj.visible === false) { this.ctx.restore(); return; }

                const points = obj.points || [];
                if (points.length < 2) { this.ctx.restore(); return; }

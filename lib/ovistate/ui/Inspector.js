...
export default class Inspector {
        ...
        return `
            <div style="padding: 15px; color: var(--text-primary);">
                ...
                <div class="inspector-section">
                    ...
                    <div class="section-title" style="font-size: 11px; color: #aaa;">Appearance</div>
                    ${this.createNumberControl('Start Size', 'particleSize', data.particleSize || 3, 1, 50)}
                    ${this.createNumberControl('End Size', 'endSize', data.endSize !== undefined ? data.endSize : (data.particleSize || 3), 0, 50)}
    ...
    static buildUIInspector(data) {
        ...
        if (data.type === 'button') {
            ...
            controls += this.createNumberControl('Min Value', 'min', data.min || 0, -500, 2000);
            controls += this.createNumberControl('Max Value', 'max', data.max || 2000, 0, 5000);
            controls += this.createNumberControl('Step', 'step', data.step || 1, 0.1, 10, 0.1);
            controls += this.createNumberControl('Current Value', 'value', data.value || 50, data.min || 0, data.max || 2000);
            ...
            controls += this.createNumberControl('Min Value', 'min', data.min || 0, -3000, 3000);
            controls += this.createNumberControl('Max Value', 'max', data.max || 100, -3000, 3000);
        ...
        if (['slider', 'checkbox', 'color_picker', 'graph', 'dropdown', 'text_input'].includes(data.type)) {
            const editor = window.oviEditor;
            const objects = editor && editor.runtime ? editor.runtime.objects : [];
            const currentTargets = data.binding?.targets || (data.binding?.targetId ? [data.binding.targetId] : []);
    ...
    static renderBehaviorSettings(data) {
        ...
        console.log(`🔍 DEBUG: buildBehaviorParameters for ${data.id} (${data.type})`, {
            raw: behaviorsRaw,
            list: behaviorsList,
        ...
        if (behaviorsList.length === 0) {
            console.log('⚠️ DEBUG: No behaviors to process.');
            return '';
        }
        if (!window.oviEditor) {
            console.error('❌ DEBUG: window.oviEditor is missing! Cannot render behavior parameters.');
            return '';
        }
        ...
        behaviorsList.forEach(behaviorId => {
            console.log(`🔍 DEBUG: Processing behavior '${behaviorId}'`);

            // Check if behavior system exists
            if (!editor.behaviorSystem) {
                console.error('❌ DEBUG: editor.behaviorSystem is missing!');
                return;
            }
            if (!editor.behaviorSystem.registry) {
                console.error('❌ DEBUG: editor.behaviorSystem.registry is missing!');
                return;
            }
            ...
            if (!behavior) {
                console.warn(`⚠️ DEBUG: Behavior '${behaviorId}' not found in registry.`);
                return;
            }

            if (!behavior.parameters) {
                console.log(`ℹ️ DEBUG: Behavior '${behaviorId}' has no parameters.`);
                return;
            }

            console.log(`✅ DEBUG: Rendering parameters for '${behaviorId}'`, Object.keys(behavior.parameters));
            ...
            params.forEach(([paramName, paramDef]) => {
                ...
                if (paramDef.type === 'slider' || paramDef.type === 'number') {
                    ...
                    const options = paramDef.options.map(opt => `<option value="${opt}" ${currentValue === opt ? 'selected' : ''}>${opt}</option>`).join('');
                    html += `
                        <div class="property-control">
                            <div class="property-label">${paramDef.label}</div>
                            <select class="behavior-param-select"
    ...
    static attachEventListeners(engine) {
        ...
        // Behavior parameter select dropdowns (Fixed: Was missing)
        ...
        document.querySelectorAll('select[data-property]:not(.behavior-param-select)').forEach(select => {
            select.addEventListener('change', (e) => {
                ...
                if (property === 'parent' && editor.selectedObject) {
                    ...
                    // Recalculate local to maintain world position
                    const newWorldParent = value ? editor.runtime.getWorldTransform(editor.runtime.getObject(value)) : { x: 0, y: 0, rotation: 0, scale: 1 };

                    // Relative vector
                    const dx = (oldWorld.x - newWorldParent.x) / newWorldParent.scale;
                    const dy = (oldWorld.y - newWorldParent.y) / newWorldParent.scale;

                    // Inverse rotate by parent
    ...
    static getInspectorStyles() {
        return `
            <style>
                ...
                select option {
                    background-color: var(--bg-secondary) !important;
                    color: #b0b0b0 !important; /* Slightly darker (dimmer) text */
                }
                select option:checked {
                    background-color: var(--accent-color, #3498db) !important; /* Attempt Blue */
    ...
    static buildBehaviorParameters(data) {
        ...
        behaviors.forEach(bId => {
            ...
            entries.forEach(([key, p]) => {
                ...
                const currentVal = (data._behaviorParams && data._behaviorParams[bId] && data._behaviorParams[bId][paramName]) !== undefined
                    ? data._behaviorParams[bId][paramName]
                    : paramDef.default;
    ...
    static generateParentOptions(editor, currentObj) {
        ...
        editor.runtime.objects.forEach(obj => {
            if (obj.id !== currentObj.id) {
                // Avoid type emitter as parents usually

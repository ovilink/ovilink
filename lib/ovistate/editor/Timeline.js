...
export default class Timeline {
    ...
    setupEvents() {
        ...
        window.addEventListener('keydown', (e) => {
            ...
            if (e.code === 'Delete' && this.selectedKeys && this.selectedKeyObj) {
                e.preventDefault();
                const frame = this.selectedKeys[0].key.t; // Assuming group drag, all share time

                // Get the active clip for the selected object
                const system = this.editor.runtime.timelineSystem;
                const activeClipName = system.activeClipName(this.selectedKeyObj);
                const timeline = this.selectedKeyObj.animations ? this.selectedKeyObj.animations[activeClipName] : this.selectedKeyObj.timeline;
                ...
                if (timeline && timeline.keys) {
                    this.selectedKeys.forEach(item => {
                        ...
                        if (timeline.keys[prop]) {
                            timeline.keys[prop] = timeline.keys[prop].filter(k => k !== item.key);
                        }
                    });
                }
                // Remove Actions
                if (timeline && timeline.actions) {
                    timeline.actions = timeline.actions.filter(k => !this.selectedKeys.find(sk => sk.key === k));
                }
            ...
            if (isCtrl && e.code === 'KeyC') {
                ...
                if (this.selectedKeys) {
                    ...
                    this.clipboard = {
                        type: 'prop',
                        keys: this.selectedKeys.map(k => ({
                            prop: k.prop,
                            val: k.key.v,
                            e: k.key.e
                        }))
                    };
                    console.log(`📋 Copied ${this.clipboard.keys.length} keys (Shortcut)`);
                } else {
                    console.warn("⚠️ No keyframes selected to copy.");
                }
            }

            // Paste (Ctrl+V)
            const targetObj = this.selectedKeyObj || this.editor.selectedObject;
            if (isCtrl && e.code === 'KeyV' && this.clipboard && targetObj) {
                ...
                if (system) {
                    this.clipboard.keys.forEach(item => {
                        ...
                        // Reuse addKeyframe to handle overwrites/sorting
                        system.addKeyframe(targetObj, item.prop, item.val, pasteFrame);
                    ...
                    // Force redrawing the whole editor might be needed if values changed visually in 3D too
                    // timeline.draw() updates UI
                    this.draw();
                    console.log(`📋 Pasted keys at frame ${pasteFrame} on ${targetObj.id}`);
                }
            }
        });
        ...
        this.canvas.addEventListener('mousedown', (e) => {
                ...
                if (obj) {
                    ...
                    if (timeline && timeline.keys) {
                        ...
                        if (hitKeys.length > 0) {
                            ...
                            // SELECTION LOGIC
                            this.selectedKeys = hitKeys;
                            this.selectedKeyObj = obj;
                            this.draw(); // To highlight? (Future)
            ...
            // 2. Fallback to Scrubbing (Deselect)
            if (x > this.sidebarWidth) {
                this.isDraggingPlayhead = true;
                this.selectedKeys = null; // Clear selection on scrub/click-away
                this.selectedKeyObj = null;
                this.scrubTo(x);
            }
        });
        ...
        window.addEventListener('mouseup', () => {
            if (this.isDraggingKeyframe) {
                ...
                if (this.draggedKeyType === 'prop') {
                    // We need to re-sort every property that was touched
                ...
                console.log(`📍 Keyframes moved to ${this.draggedKeys[0].key.t}`);
            }

            this.isDraggingPlayhead = false;
            this.isDraggingKeyframe = false;
            this.draggedKeys = null; // Clear array
    ...
    drawTracks(ctx, w, startX) {
        ...
        for (let i = 0; i < objects.length; i++) {
            ...
            if (timeline.keys) {
                for (const prop in timeline.keys) {
                    this.drawKeyframes(ctx, timeline.keys[prop], y, startX);
                }
            }

            // Draw Action Keyframes
            if (timeline.actions) {
                this.drawActionKeys(ctx, timeline.actions, y, startX);
            }
        }
    }

    drawKeyframes(ctx, keys, y, startX) {
        ...
        keys.forEach(k => {
            ...
            if (this.selectedKeys) {
                isSelected = this.selectedKeys.some(sk => sk.key === k);
            }
    ...
    drawActionKeys(ctx, actions, y, startX) {

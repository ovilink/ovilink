...
const LIBS = {
    ...
    class Particle {
        ...
        reset() {
            ...
            this.life = 0; this.maxLife = 1; this.color = '#ffffff';
            this.size = 2; this.active = false; this.alpha = 1;
        }
    }
    class ParticleSystem {
        constructor() {
            this.pool = []; this.activeParticles = []; this.maxParticles = 500;
            for (let i = 0; i < this.maxParticles; i++) this.pool.push(new Particle());
        }
        spawn(x, y, config) {
            let p = this.pool.length > 0 ? this.pool.pop() : null;
            if (!p) return;
            ...
            p.life = p.maxLife; p.color = config.color || '#ff0000';
            p.size = config.size || 3; p.alpha = 1;
        ...
        update(dt) {
            for (let i = this.activeParticles.length - 1; i >= 0; i--) {
                ...
                p.life -= dt;
                if (p.life <= 0) {
                    p.active = false; this.pool.push(p);
            ...
            for (let i = 0; i < this.objects.length; i++) {
                ...
                for (let j = i + 1; j < this.objects.length; j++) {
                    ...
                    // AABB-AABB
                        ...
                        else { bA.w = a.width||40; bA.h = a.height||40; }

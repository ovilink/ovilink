        ...
        this.objects.forEach(obj => {
            if (obj.behaviors && obj.behaviors.length > 0) {
                obj.behaviors.forEach(behaviorId => {
                    ...
                    if (behavior) behavior(obj, this, dt);
                });
            }
        });
        ...
        if (this.enablePhysics) {
            this.objects.forEach(obj => {
                if (obj.physics && obj.physics.enabled) {
                    obj.physics.velocity.y += this.gravity;
                    ...
                    if (obj.type === 'circle') {
                        if (obj.y + obj.radius > this.height) {
                            ...
                            obj.physics.velocity.y *= -((obj.physics.bounciness||0.8));
                        }
                        if (obj.x + obj.radius > this.width || obj.x - obj.radius < 0) {
                        ...
                        if (obj.x + obj.width/2 > this.width || obj.x - obj.width/2 < 0) {
                            obj.physics.velocity.x *= -1;
                        }
                        ...
                        if (obj.x + half > this.width || obj.x - half < 0) {
                            obj.physics.velocity.x *= -1;
                        }
                    }
                }
...
class RuntimeUI {
    ...
    triggerAction(action, target, actionId) {
        ...
        switch(action) {
            ...
            case 'jump': if(target.physics) target.physics.velocity.y = -10; break;
...
<body>
    ...
    <script type="module">
        ...
        const runtime = new OviStateRuntime(container, {
            ...
            enablePhysics: GAME_DATA.physics.enablePhysics,
            // Core.js uses per-frame gravity. If value is high (m/s^2 style 9.8), scale it to per-frame (~0.5)
            gravity: (GAME_DATA.physics.gravity > 2) ? GAME_DATA.physics.gravity * 0.05 : GAME_DATA.physics.gravity
        });

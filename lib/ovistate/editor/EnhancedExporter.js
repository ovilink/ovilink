...
class OviStateRuntime {
    ...
    getObjectBounds(obj) {
        ...
        if (children.length > 0) {
            children.forEach(child => {
                ...
                if (childBounds.minX === Infinity) return;
            ...
            this.objects.forEach(obj => {
                if (obj.physics && obj.physics.enabled) {
                    ...
                    if (obj.y + bounds.maxY > this.height) { 
                        ...
                        obj.physics.angularVelocity *= 0.9;
                    }
                ...
                collsA.forEach(ca => {
                    collsB.forEach(cb => {
                        ...
                        if (ca.isCircle && cb.isCircle) {
                            ...
                            const circle = ca.isCircle ? ca : cb, rect = ca.isCircle ? cb : ca;
                            const dx = circle.world.x - rect.world.x, dy = circle.world.y - rect.world.y;
                            const rad = -rect.world.rotation * Math.PI / 180, cos = Math.cos(rad), sin = Math.sin(rad);
    ...
    render() {
        ...
        // Sort by zIndex
        this.objects.slice().sort((a, b) => (a.zIndex || 0) - (b.zIndex || 0)).forEach(obj => {
            const world = this.getWorldTransform(obj);
            ...
            if (obj.type === 'vector_path') {
                ...
                this.ctx.fillStyle = this.resolvePaint(obj.fill || '#3498db', obj);
                this.ctx.fill();
                ...
                this.ctx.fillStyle = this.resolvePaint(obj.fill || '#2ecc71', obj);
                ...
                if (obj.verticalAlign === 'top') {
                    startY = 0 - (obj.height || 0)/2 + (lineHeight / 2); 
                }
...
function registerTransform(registry) {
    ...
    registry.register('wheel_rolling', {
        init(obj, rt, reg) {
            ...
            obj._wO = obj.rotation;
            const tId = reg.getParameter(obj, 'wheel_rolling', 'targetId');
            const t = tId ? (rt.getObject(tId) || rt.getObject(obj.parent)) : rt.getObject(obj.parent);
            obj._pSX = t ? (t.x || 0) : 0;
            obj._wI = true;
        },
        update(obj, dt, rt, reg) {
            if(!obj._wI) this.init(obj, rt, reg);
            const tId = reg.getParameter(obj, 'wheel_rolling', 'targetId');
            const t = tId ? (rt.getObject(tId) || rt.getObject(obj.parent)) : rt.getObject(obj.parent);
            if(!t) return;
            const auto = reg.getParameter(obj,'wheel_rolling','autoRadius')!==false;
            const rad = reg.getParameter(obj,'wheel_rolling','radius')||30;
            const rev = reg.getParameter(obj,'wheel_rolling','reverse')===true;
            let r = rad;
            if(auto) {
            ...
            const distMoved = (t.x||0) - (obj._pSX||0);
            const circ = 2*Math.PI*r;
            const deg = (distMoved/circ)*360;
            obj.rotation = obj._wO + (rev?-deg:deg);
        }
    });
}

...
export default class OviStateEditor {
    ...
    create() {
        ...
        bgInput.type = 'color';
        bgInput.value = this.simulationData.canvas.background || '#1e1e1e';
        ...
        bgInput.addEventListener('input', (e) => {
            // CRITICAL: Update Source Data for Export
            if (this.simulationData && this.simulationData.canvas) {
                this.simulationData.canvas.background = e.target.value;
            }
        ...
        window.addEventListener('mouseup', () => {
            ...
            if (this.runtime && (this.runtime._draggingPath || this.runtime.isMouseDown)) {
                if (this.runtime._draggingPath) changed = true;
                ...
                if (obj.type === 'path' && obj.points) {
                    ...
                    if (hitIdx !== -1) {
                        if (obj.points.length > 2) { // Minimum 2 points
                            ...
                            this.runtime._draggingPath = null;
                            this.runtime._draggingVertexIndex = null;
                            this.saveState(); // SAVE: Vertex Deleted
                            return; // Handled
                        }
                    }
                    ...
                    for (let k = 1; k <= steps; k++) {
                        ...
                        if (dist <= 6) { // Tolerance
                            ...
                            this.saveState(); // SAVE: Vertex Inserted
                            return;
                        }
                        prevP = currP;
                    }
    ...
    reset() {
        ...
        // SAVE: Any changes during interaction/recording
        this.saveState();
    }
    ...
    saveState() {
        ...
        // 1. Sync latest data from Runtime back to simulationData
        if (this.runtime) {
            this.runtime.objects.forEach(runtimeObj => {
                const sourceObj = this.simulationData.objects.find(o => o.id === runtimeObj.id);
                if (sourceObj) {
                    // Sync transform
                    sourceObj.x = runtimeObj.x;
                    sourceObj.y = runtimeObj.y;
                    sourceObj.rotation = runtimeObj.rotation;
                    sourceObj.scale = runtimeObj.scale || 1;
                    sourceObj.opacity = runtimeObj.opacity !== undefined ? runtimeObj.opacity : 1;

                    // Sync Animations (Keyframes)
                    if (runtimeObj.animations) {
                        sourceObj.animations = JSON.parse(JSON.stringify(runtimeObj.animations));
                    }
                    if (runtimeObj.timeline) {
            ...
            // 2. Sync Global Timeline Metadata
            const ts = this.runtime.timelineSystem;
            if (ts) {
                this.simulationData.timeline = {
                    totalFrames: ts.totalFrames,
                    fps: ts.fps,
                    loop: ts.loop,
                    activeClips: Object.fromEntries(ts.activeClips) // Convert Map to Object
                };
            }
        }

        // 3. Capture a clean snapshot of the data
        try {
            const snapshot = JSON.parse(JSON.stringify(this.simulationData));
    ...
    undo() {
        ...
        const prevState = this.history.undo(this.simulationData);
        if (prevState) {
    ...
    redo() {
        ...
        const nextState = this.history.redo(this.simulationData);
        if (nextState) {
    ...
    loadState(state) {
        ...
        this.runtime.clear();
        this.selectedObjects.clear();
        if (this.overlayZone) this.overlayZone.innerHTML = '';

        // 3. Re-initialize Objects
        if (state.objects) {
            state.objects.forEach(obj => {
                this.runtime.addObject(obj);
                // Re-wire parent/child if needed (not deep for V1)
            });
        }

        // 4. Re-initialize Controls
        if (state.controls) {
            state.controls.forEach(control => {
                this.runtime.addControl(control);
                this.renderUIComponent(control, this.overlayZone);
            });
        }

        // 5. Re-apply Canvas Settings
        if (state.canvas) {
            this.resizeCanvas(state.canvas.width || 800, state.canvas.height || 600);
            if (this.runtime.config) this.runtime.config.background = state.canvas.background;
            if (this.runtime.canvas) this.runtime.canvas.style.backgroundColor = state.canvas.background;
        }

        // 5b. Re-apply Timeline Metadata
        if (state.timeline && this.runtime.timelineSystem) {
            const ts = this.runtime.timelineSystem;
            ts.totalFrames = state.timeline.totalFrames || 300;
            ts.fps = state.timeline.fps || 30;
            ts.loop = state.timeline.loop !== undefined ? state.timeline.loop : true;

            // Restore activeClips Map
            if (state.timeline.activeClips) {
                ts.activeClips = new Map(Object.entries(state.timeline.activeClips));
            }

            // Sync Timeline UI
            if (this.timeline) {
                this.timeline.updateClipSelector();
                this.timeline.draw();
            }
        }
        ...
        // 7. Sync Scene Registry (OviHub) - Optional for V1, but good practice
        this.syncAllToRegistry();
    }
}

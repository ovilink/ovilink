...
export class OviStateRuntime {
    ...
    executeAction(action, target, config = {}) {
        ...
        switch (action) {
            ...
            case 'start_behavior':
                if (actionId) this.setBehaviorStateByEventId(target, actionId, true);
                break;
            case 'stop_behavior':
    ...
    getWorldTransform(obj, visited = new Set()) {
        if (visited.has(obj.id)) {
            console.warn(`[Runtime] Circular parent detected for ${obj.id}`);
        ...
        if (obj.parent) {
            ...
            if (parent) {
                const pt = this.getWorldTransform(parent, visited);
    ...
    updateInput() {
        ...
        this.objects.slice().sort((a, b) => (b.zIndex || 0) - (a.zIndex || 0)).forEach(obj => {
            ...
            if (obj.type === 'vector_path') {
                try {
                    // Lazy init: Create Path2D on demand
                    // Check if _path2d is MISSING OR INVALID (e.g. from JSON deserialization)
                    if ((!obj._path2d || !(obj._path2d instanceof Path2D)) && obj.pathData) {
                        obj._path2d = new Path2D(obj.pathData);
                    }

                    // Robust check: Ensure it is a valid Path2D before usage
                    if (obj._path2d && obj._path2d instanceof Path2D) {
                        let alx = lx, aly = ly;
                        if (obj.renderOffset) { alx -= obj.renderOffset.x; aly -= obj.renderOffset.y; }
                        // Explicit 'nonzero' fill rule to prevent argument shifting if browser is confused
                        isHit = this.ctx.isPointInPath(obj._path2d, alx, aly, 'nonzero');
                    }
                } catch (e) {
                    console.warn(`[RuntimeCore] Path hit test error for ${obj.id}:`, e);
                }
            ...
            if (this.isMouseDown && !this.clickProcessed && isHit && !this._clickHandled) {
                ...
                // --- NEW: Magic Drag for Recording ---
                // If we are recording, the object becomes draggable even without the behavior
                if (this.timelineSystem && this.timelineSystem.isRecording) {
    ...
    update(dt) {
        ...
        // --- NEW: Magic Drag Logic (Core Movement) ---
        if (this._draggingObj && this.timelineSystem && this.timelineSystem.isRecording) {
            // If the object DOES NOT have a draggable behavior, we move it manually here
            const hasDraggable = this._draggingObj.behaviors && this._draggingObj.behaviors.includes('draggable');
            if (!hasDraggable) {
                this._draggingObj.x = this.mouseX;
                this._draggingObj.y = this.mouseY;
                if (this._draggingObj.physics) this._draggingObj.physics.velocity = { x: 0, y: 0 };
            }
        }
        ...
        // Procedural Updates (for objects like ripples)
        this.objects.forEach(obj => {
    ...
    getRecursiveOpacity(obj) {
        ...
        let steps = 0;
        while (p && steps < 50) {
            const parent = this.objects.find(o => o.id === p);
            if (parent) {
        ...
        try {
            ...
            sorted.forEach(obj => {
                ...
                if (obj.type === '3d_model') {
                    ...
                    if (obj.shadowBlur > 0) {
                        this.ctx.shadowBlur = obj.shadowBlur;
                        this.ctx.shadowColor = obj.shadowColor || 'black';
                    } else {
                    ...
                    if (obj.showInExport === false || obj.visible === false) { this.ctx.restore(); return; }
                    ...
                    try {
                        ...
                        if (obj._path2d && obj._path2d instanceof Path2D) {
                            ...
                            if (obj.fill && obj.fill !== 'none') {
                                ...
                                this.ctx.fill(obj._path2d, 'nonzero');
                            }
                        ...
                        console.warn(`[RuntimeCore] Path render error for ${obj.id}:`, e);
                    }
    ...
    clear() {
        ...
        // Clear DOM Layers for 3D
        this._ovi3dLayers.forEach(layer => {
            if (layer.container && layer.container.parentNode) {
                layer.container.parentNode.removeChild(layer.container);
            }
        });
        this._ovi3dLayers.clear();

...
function registerPath(registry) {
    registry.register('follow_path', {
        ...
        update(obj, dt, runtime, registry) {
            ...
            // Check both object property and behavior parameter for orient
            const orientParam = registry.getParameter(obj, 'follow_path', 'orient');
            const orientToPath = (obj.orientToPath !== undefined) ? obj.orientToPath : orientParam;

            if (orientToPath && obj._pathProgress < 1) {
    ...
    registry.register('scrub_path', {
        update(obj, dt, runtime, registry) {
            ...
            const progress = registry.getParameter(obj, 'scrub_path', 'progress');
            const orientParam = registry.getParameter(obj, 'scrub_path', 'orient');
            const tension = pathObj.tension !== undefined ? pathObj.tension : 0.5;

            const pos = runtime.getSplinePoint(pathObj.points, tension, progress, pathObj.closed);

            // Check both object property and behavior parameter for orient
            const orientToPath = (obj.orientToPath !== undefined) ? obj.orientToPath : orientParam;

            if (orientToPath && progress < 1) {

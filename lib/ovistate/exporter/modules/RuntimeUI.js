...
export class RuntimeUI {
    ...
    triggerAction(control, target) {
        ...
        const actionId = control.binding.actionId;

        console.log(`⚡ [UI] Triggering ${action} on ${target.id}`);
        ...
        switch (action) {
            ...
            case 'emit_action':
                if (actionId) {
                    this.runtime.lastAction = actionId;
                }
                break;
            ...
            case 'set_variable':
                if (control.binding.variableName && control.binding.value !== undefined && this.runtime.setVariable) {
                    let val = control.binding.value;
                    if (!isNaN(val) && val !== '') val = Number(val);

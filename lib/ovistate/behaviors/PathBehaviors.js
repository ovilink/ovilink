...
export function registerPathBehaviors(registry) {
    ...
    // Scrub Path - Manual progress control (useful for UI binding)
    registry.register('scrub_path', {
        name: 'Scrub Path',
        category: 'path',
        icon: '🎚️',
        description: 'Control path position via progress parameter',
        parameters: {
            pathId: { type: 'select', options: 'objects:path', label: 'Path Object' },
            progress: { type: 'slider', min: 0, max: 1, default: 0, step: 0.01, label: 'Progress' },
            orient: { type: 'toggle', default: true, label: 'Orient to Path' }
        },
        update(obj, dt, runtime, registry) {

...
export function registerPathBehaviors(registry) {
    ...
    registry.register('follow_path', {
        ...
        init(obj) {
            if (!obj._pathProgress) obj._pathProgress = 0;
        },
        update(obj, dt, runtime, registry) {
            ...
            // Advance progress
            obj._pathProgress += dt * speed;
            if (obj._pathProgress > 1) {
                if (loop) obj._pathProgress %= 1;
                else obj._pathProgress = 1;
            }

            // Get point on spline
            const pos = runtime.getSplinePoint(pathObj.points, pathObj.tension || 0.5, obj._pathProgress, pathObj.closed);

            // If orient to path, calculate tangent
            if (orient && obj._pathProgress < 1) {
                const nextT = Math.min(1, obj._pathProgress + 0.01);
                const nextPos = runtime.getSplinePoint(pathObj.points, pathObj.tension || 0.5, nextT, pathObj.closed);

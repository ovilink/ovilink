...
export function registerTriggerBehaviors(registry) {
    ...
    // Play Animation - Play a clip on Action ID or Click
    registry.register('play_animation', {
        name: 'Play Animation',
        category: 'interaction',
        icon: '🎬',
        description: 'Play an animation clip',
        parameters: {
            clipName: { type: 'text', default: 'default', label: 'Clip Name' },
            loop: { type: 'checkbox', default: true, label: 'Loop' },
            onFinishActionId: { type: 'text', default: '', label: 'On Finish Action' }
        },
        update(obj, dt, rt, reg) {
            // Check both standard Activation Modes (via BehaviorState) 
            // and legacy ActionID parameter for convenience.
            const actMode = reg.getParameter(obj, 'play_animation', 'activationMode') || 'on_enter';
            ...
            // In 'on_enter' mode, it starts Active but we want to consume it.
            // BehaviorSystem sets obj._behaviorState[behaviorId] = true.
            ...
            if (rt.timelineSystem && clipName) {
                const onFinish = onFinishActionId ? () => {
                    console.log(`🏁 Animation "${clipName}" finished. Triggering: ${onFinishActionId}`);
                    rt.emitAction(onFinishActionId);
                } : null;

                rt.timelineSystem.playClip(obj, clipName, loop, onFinish);

                // Consumption logic: Deactivate state to prevent re-play every frame
                // This works for both 'on_enter' and 'on_click' modes.

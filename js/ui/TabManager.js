...
export default class TabManager {
    constructor() {
        ...
        this.welcomeScreen = document.getElementById('welcome-screen'); // NEW
    ...
    init() {
        ...
        if (this.editorArea) {
            // Keep welcome screen, clear others
    ...
    updateWelcomeScreen() {
        if (this.welcomeScreen) {
            // Show welcome screen ONLY if there are NO tabs open
            this.welcomeScreen.style.display = this.tabs.size === 0 ? 'block' : 'none';
        }
    }

    // ... (rest of setupOverflowDetection and setActiveEditor unchanged)
    ...
    openTab(title, pluginId, content, editorInstance = null) {
        ...
        this.updateWelcomeScreen(); // NEW

        return id;
    }

    // ... (activateTab unchanged)
